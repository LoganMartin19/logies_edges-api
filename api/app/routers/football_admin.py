...
def prime_players(day: str = Query(..., description="UTC YYYY-MM-DD"), days: int = Query(1, ge=1, le=7), db: Session = Depends(get_db)):
    ...
    for f in fixtures:
        try:
            ...
            if season and home_id: get_team_season_players_cached(db, home_id, season)
            if season and away_id: get_team_season_players_cached(db, away_id, season)
            get_fixture_players_cached(db, pfx)
            primed += 1
        except Exception:
            continue
    return {"day": day, "days": days, "primed": primed}