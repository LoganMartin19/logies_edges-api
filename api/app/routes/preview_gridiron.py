...
router = APIRouter(prefix="/ai/preview/gridiron", tags=["AI Preview (NFL)"])
...
def _build_prompt(fx: Fixture, probs: dict) -> str:
    ...
    return f"""
Write a concise NFL game preview (5–7 lines) for {home} vs {away}.
Use neutral tone. Use ONLY the provided probabilities and note recent form if available.
Finish with one 'what decides it' line.

Model probabilities (moneyline): {home} {p_home}, {away} {p_away}.
If probabilities are missing, write a generic matchup frame without inventing data.
""".strip()
...
def generate_ai_preview(fixture_id: int = Query(...), overwrite: bool = Query(False), db: Session = Depends(get_db)):
    ...
    resp = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[{"role":"system","content":"You are a neutral NFL analyst."},{"role":"user","content":prompt}],
    ...
    if existing:
        ...
        db.add(AIPreview(fixture_id=fixture_id, day=today, sport="nfl", comp=fx.comp, preview=text, model="gpt-4o-mini", tokens=tokens))
    db.commit()
    return {"ok": True, "fixture_id": fixture_id, "preview": text, "tokens": tokens}

@router.post("/generate/daily")
def generate_daily_ai_previews(day: str = Query(date.today().isoformat()), sport: str = Query("nfl"), overwrite: bool = Query(False), db: Session = Depends(get_db)):
    start = datetime.fromisoformat(day+"T00:00:00+00:00")
    end = start + timedelta(days=1)
    fixtures = db.query(Fixture).filter(Fixture.sport.in_(["nfl","gridiron"]), Fixture.kickoff_utc >= start, Fixture.kickoff_utc < end).all()
