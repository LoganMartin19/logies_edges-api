...
def _build_prompt(fixture: Fixture, form_data: Dict[str, Any], n: int) -> str:
    """Build concise analyst-style preview prompt."""
    ...
    h_w, h_d, h_l = (
        int(home_form.get("wins", 0)),
        int(home_form.get("draws", 0)),
        int(home_form.get("losses", 0)),
    )
    a_w, a_d, a_l = (
        int(away_form.get("wins", 0)),
        int(away_form.get("draws", 0)),
        int(away_form.get("losses", 0)),
    )
...
def generate_ai_preview(
    ...
    """Generate or return cached AI preview."""
    ...
    form_data = get_fixture_form_summary(db, fixture_id, n=n)
    if not form_data:
...
def debug_form_data(fixture_id: int, db: Session = Depends(get_db)):
    ...
    form = get_fixture_form_summary(db, fixture_id, n=5)
    return {
        "fixture_id": fixture_id,
